...
# --- PHASE 3: EVALUATION (LOSEN, LESEN & FRAGEBOGEN) ---
    ...
    if st.session_state.aktuelles_transkript_file is None:
        ...
        # --- RECHTE SPALTE: EINSCHÄTZUNG (OHNE ST.FORM) ---
        with col_right:
        ...
        # --- BEREICH UNTERHALB DER BEIDEN SPALTEN ---
        st.write("---")
        anmerkungen = st.text_area("Gibt es noch sonstige Auffälligkeiten oder Bemerkungen zur Person? (Optional)", max_chars=500)
        
        # Der Button sitzt jetzt zentriert und außerhalb der Boxen
        submit_button = st.button("Formular absenden", type="primary", use_container_width=True)
